...
object InitGenerator {
  ...
  def initUserBaseFeature(ss: SparkSession): Unit = {
    ...
    var dropTableSql =
      """
        |drop table if exists dwd.dwd_user_base_feature
        |""".stripMargin
    ss.sql(dropTableSql)
    ...
    var createTableSql =
      """
        |create table if not exists dwd.dwd_user_base_feature (
        |distinct_id string,
        |gender string,
        |age string,
        |phonenum string,
    ...
    val userBaseFeatureDF = ss.read.format("org.apache.spark.sql.execution.datasources.csv.CSVFileFormat")
      .option("sep", ",") //数据的分隔符
      .option("header", "true") //首行是否字段元数据
      .load(path)

    userBaseFeatureDF.cache()
    userBaseFeatureDF.write.mode(SaveMode.Overwrite).saveAsTable("dwd_user_base_feature")
